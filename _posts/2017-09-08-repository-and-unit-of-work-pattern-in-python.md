...
In the [previous part](http://io.made.com/blog/2017-09-07-introducing-command-handler.html) of
...
extending our system through **composition**. What makes this a 
ports-and-adapters architecture is the idea that there is an internal world 
consisting of the domain model (our ThresholdDetectionCircuit), and an
external world that drives the domain model through well-defined ports. How
does all of this relate to databases?
...
fact, the IssueLog is an instance of a common design pattern: it's a
[Repository](https://martinfowler.com/eaaCatalog/repository.html). A
repository is an object that hides the details of persistent storage by
presenting us with an interface that looks like a collection. We should be
able to add new things to the repository, and get things out of the
repository, and that's essentially it.
...
object, so it's part of our Adapter layer. We could define a different
adapter for use in unit tests.
...
commonly used with another pattern, the 
[Unit of Work](https://martinfowler.com/eaaCatalog/unitOfWork.html). A unit
of work represents
